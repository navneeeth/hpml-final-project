...
def main():
    ...
    train_root = './datasets_dir/ground_truth/'
    train_dataset = MET_database(root = train_root,mini = True,transform = extraction_transform,im_root = './datasets_dir/')
    '''
    ...
    test_dataset = MET_queries(root = query_root,test = True,transform = extraction_transform,im_root = './datasets_dir/images/test_images/')
    #val_dataset = MET_queries(root = query_root,transform = extraction_transform,im_root = args.im_root)
    val_dataset = MET_queries(root = query_root,transform = extraction_transform,im_root = './datasets_dir/images/test_images/')
    ...
    if netpath is not None:
	    ...
	    if network_variant == 'r18INgem':
		    ...
		    net = torch.nn.Sequential(*list(model.children())[:-2],GeM(),F.normalize)
		    net.meta = {
    ...
    train_descr = extract_embeddings(net,train_loader,ms = scales,msp = 1.0,print_freq=1)
	...
	#save descriptors
    with open(exp_dir+"descriptors2.pkl", 'wb') as data:
